...
def simulateShackHartmannReconstruction():
    ...
    for x,y in zip(lensletXXVector,lensletYYVector):
        plt.plot(coord2index(x),coord2index(y),'ks',markersize=3)
    ...
    comparison_plot_type='scatter'
    xax = np.arange(len(cVec))
    if comparison_plot_type=='scatter':
        ph1=plt.plot(xax,cVec,'go',markersize=6,alpha=0.75)
        
        ph2=plt.plot(xax,rVec,'bs',markersize=6,alpha=0.75)
